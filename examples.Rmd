...
ma_cities <- tibble(
  name = c("Northampon", "Boston", "Springfield", "Worcestor"), 
  lat = c(42.3251, 42.3601, 42.1015, 42.2626), 
  lon = c(-72.6412, -71.0589, -72.5898, -71.8023)
)

ma_cities_sf <- ma_cities %>%
  st_as_sf(coords = c("lon", "lat"), crs = 4326)

USA_map <-  maps::map("state", region = c('massachusetts'), plot = FALSE, fill = TRUE)
plot(USA_map)
...
census_api_key("a379ae402a43ea938fe070e1a32ff92a31b8440e")
mass_pop_orig <- 
  tidycensus::get_acs(
    geography = "county", 
